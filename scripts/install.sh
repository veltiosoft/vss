...
if [ "$OS" = "Windows_NT" ]; then
	...
	if [ $# -eq 0 ]; then
		vss_uri="https://github.com/veltiosoft/vss/releases/latest/download/vss_${target}.zip"
	else
		vss_uri="https://github.com/veltiosoft/vss/releases/download/${1}/vss_${target}.zip"
	fi
else
	case $(uname -sm) in
	"Darwin x86_64")
	...
	*) target="linux_amd64" ;;
	...
	if [ $# -eq 0 ]; then
		vss_uri="https://github.com/veltiosoft/vss/releases/latest/download/vss_${target}.${ext}"
	else
		vss_uri="https://github.com/veltiosoft/vss/releases/download/${1}/vss_${target}.${ext}"
	fi
fi
